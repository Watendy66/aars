...
package aars.venus.editors.jeditsyntax.tokenmarker;

import aars.venus.editors.jeditsyntax.*;
import aars.mips.instructions.*;
import aars.assembler.*;
...
public class MIPSTokenMarker extends TokenMarker {
    ...
    public static KeywordMap getKeywords() {
        if (cKeywords == null) {
            ...
            for (int i = 0; i < instructionSet.size(); i++) {
                cKeywords.add(((aars.mips.instructions.Instruction) instructionSet.get(i)).getName(), Token.KEYWORD1);
            }
            // add assembler directives
            java.util.ArrayList directiveSet = aars.assembler.Directives.getDirectiveList();
            for (int i = 0; i < directiveSet.size(); i++) {
                cKeywords.add(((aars.assembler.Directives) directiveSet.get(i)).getName(), Token.KEYWORD2);
            }
            // add integer register file
            aars.mips.hardware.Register[] registerFile = aars.mips.hardware.RegisterFile.getRegisters();
            ...
            aars.mips.hardware.Register[] coprocessor1RegisterFile = aars.mips.hardware.Coprocessor1.getRegisters();
            ...
            switch (token) {
                case Token.NULL:
                     ...
                     */
                            ...
                            try {
                                validIdentifier = aars.assembler.TokenTypes.isValidIdentifier(new String(array, lastOffset, i1 - lastOffset - 1).trim());
    ...
    public ArrayList getTokenExactMatchHelp(Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.KEYWORD1) {
            ArrayList instrMatches = aars.Globals.instructionSet.matchOperator(tokenText);
            if (instrMatches.size() > 0) {
                int realMatches = 0;
                matches = new ArrayList();
                for (int i = 0; i < instrMatches.size(); i++) {
                    Instruction inst = (Instruction) instrMatches.get(i);
                    if (aars.Globals.getSettings().getExtendedAssemblerEnabled() || inst instanceof BasicInstruction) {
    ...
    public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.NULL) {
            ...
            if (keywordTokenText == null && trimmedTokenText.length() > 0) {
                ...
                } else if (trimmedTokenText.length() >= aars.Globals.getSettings().getEditorPopupPrefixLength()) {
    ...
    private ArrayList getTextFromInstructionMatch(String tokenText, boolean exact) {
        ...
        if (exact) {
            matches = aars.Globals.instructionSet.matchOperator(tokenText);
        } else {
            matches = aars.Globals.instructionSet.prefixMatchOperator(tokenText);
        }
        if (matches == null) {
            return null;
        }
